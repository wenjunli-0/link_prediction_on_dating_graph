...
distinct_f_id_be_clicked_by_m_not_msg = []
for i in range(f_id_list.shape[0]):
    ...
    if not (f_id_in_msg_m == f_id).any():
        if (f_id_in_click_m == f_id).any():
            distinct_f_id_be_clicked_by_m_not_msg.append(f_id)

np.savetxt("./march_26/distinct_f_id_be_clicked_by_m_not_msg.csv", distinct_f_id_be_clicked_by_m_not_msg, delimiter=',', fmt='%d')
# '''
...
distinct_m_id_be_clicked_by_f_not_msg = []
for i in range(m_id_list.shape[0]):
    ...
    if not (m_id_in_msg_f == m_id).any():
        if (m_id_in_click_f == m_id).any():
            distinct_m_id_be_clicked_by_f_not_msg.append(m_id)

np.savetxt("./march_26/distinct_m_id_be_clicked_by_f_not_msg.csv", distinct_m_id_be_clicked_by_f_not_msg, delimiter=',', fmt='%d')
